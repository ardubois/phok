...
def get_gnx({:nx, type, shape, name , ref}) do
  ...
  %Nx.Tensor{data: %Nx.BinaryBackend{ state: get_matrex_nif(ref,l,c)}, type: type, shape: shape, names: name}
end
...
def spawn_jit(k,_t,_b,l) do
 ...
 #IO.puts "JIT"
  ...
  out = :os.cmd('echo \'#{inspect prog}\' | nvcc -x cu --shared --compiler-options \'-fPIC\' -o priv/Elixir.#{kernel_name}.so -')
